...
const transporter = nodemailer.createTransport({
  ...
  exports.verifyOtp = (req , res)=>{
    ...
    UserSchema.find({email:email}).then((d1)=>{
        if(d1.length > 0){
            ...
            res.status(200).send({status : 200 , message : "Invalid Email"})
        }
    }).catch((err)=>{
        res.status(500).send({status : 500 , message : "Soemthing Went Wrong || Try Again" })
    })
  ...
  exports.forgotPassword =(req , res )=>{
    ...
    UserSchema.find({email : u_mail}).then((d1)=>{
        if(d1.length > 0){
            OtpSchema.insertMany({u_id : d1[0]._id , email :d1[0].email,  otp : otp , time : Number(new Date(Date.now())) }).then((o_data)=>{
                if(o_data.length > 0){
                    ...
                    res.status(400).send({status : 400 , message : "OTP Sent Failed" , mailId  : null})
                }
            }).catch((err)=>{
        ...
        res.status(500).send({status :  500 , message : "Something Went Wrong !! Please Try Again", mailId : null})
    })
        ...
        if(result1.length > 0)
        {
            CartSchema.deleteMany({u_id:u_id}).then((re_2)=>{
                if(re_2.deletedCount > 0){
                    ...
                    res.status(400).send({status : 400 , message : "Order Not Placed || Please Try Again"})
                }
            }).catch((err)=>{
                console.log(err)
                res.status(500).send({status :  500 , message : "Something Went Wrong !! Please Try Again"})
        ...
        if(cart_arr.length>0){
            ProductSchema.find({}).then((pr_res)=>{
                ...
                if(pr_res.length>0){
                    ...
                    res.status(200).send({status :  200 ,  data: new_data , count:cart_arr.length, message : "Product Added into Cart"})
                }
                else{
                    res.status(200).send({status :  200 , data:[], count:0 ,  message : "Product Not Added into Cart|| Please Try Again"})
                }
            }).catch((err)=>{
        res.status(500).send({status :  500 , message : "Something Went Wrong !! Please Try Again"})
    })
...
exports.getCartCountByUserID = (req,res)=>{
    ...
    CartSchema.find({u_id: u_id}).then((result)=>{
        if(result.length>0){
            ...
            res.status(200).send({status :  200 , data:result, count:result.length,  message : "Product Not Added into Cart|| Please Try Again"})
        }
    }).catch((err)=>{
        res.status(500).send({status :  500 , message : "Something Went Wrong !! Please Try Again"})
    })
